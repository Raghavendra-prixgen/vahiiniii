...
class PurchaseRequestLineMakePurchaseOrder(models.TransientModel):
    ...
    sync_data_planned = fields.Boolean(
        string="Match existing Purchase Order lines by Scheduled Date",
        help=(
            "When checked, Purchase Order lines on the selected purchase order are only reused "
    ...
    def onchange_purchase_order_id(self):
        for rec in self:
            if rec.purchase_order_id:
                rec.write({'supplier_id':rec.purchase_order_id.partner_id.id})

    @api.model
    def _prepare_item(self, line,purchase_done_quantity):
        return {
            "line_id": line.id,
            "request_id": line.request_id.id,
            "product_id": line.product_id.id,
            "name": line.name or line.product_id.name,
            "product_qty": line.product_qty - purchase_done_quantity,
            "quantity": line.product_qty - purchase_done_quantity,
    ...
    def _check_valid_request_line(self, request_line_ids):
        picking_type = False
        purchase_request_line_ids = self.env["purchase.request.line"].browse(request_line_ids)
        company_ids = purchase_request_line_ids.request_id.mapped('company_id')
        ...
        for line in purchase_request_line_ids:
            ...
            if line.request_id.state not in ("approved",'partial'):
                raise UserError(_("Purchase Request %s is not approved") % line.request_id.name)
            is_done_list = [True if line.purchased_qty == line.product_qty else False for line in line.request_id.line_ids]
            if all(is_done_list):
                raise UserError(_("The purchase has already been completed."))
    ...
    def get_items(self, request_line_ids):
        ...
        for line in request_lines.filtered(lambda x:x.y_is_short_closed == False):
            purchase_done_quantity = sum(line.purchase_lines.filtered(lambda x:x.state in ('done','purchase')).mapped('product_qty'))
            if line.product_qty != purchase_done_quantity and (line.product_qty - purchase_done_quantity) > 0:
                items.append([0, 0, self._prepare_item(line,purchase_done_quantity)])
        return items
    ...
    @api.model
    def _prepare_purchase_order(self, company, origin,picking_type_id):
        ...
        data = {
            ...
            "fiscal_position_id": supplier.property_account_position_id.id  or False,
    ...
    def _prepare_purchase_order_line(self, po, item):        
        ...
        if item.product_uom_id != product_uom:
            qty = item.product_uom_id._compute_quantity(
                item.quantity,product_uom)
        else:
            qty = item.quantity
    ...
    def make_purchase_order(self):
        ...
        purchase_id = self.purchase_order_id
        
        purchase_request_ids = self.item_ids.mapped('request_id')
        company_ids = purchase_request_ids.mapped('company_id')
        if len(company_ids) > 1:
            raise UserError(_("You have to select lines from the same company."))
        company_id = company_ids[:1]

        picking_type_ids = purchase_request_ids.mapped('picking_type_id')
        if len(picking_type_ids) > 1:
            raise UserError(_("You have to select lines from the same Deliver To."))
        picking_type_id = picking_type_ids[:1]

        if self.item_ids.filtered(lambda x:x.quantity <= 0.0):
            raise UserError(_("Quantity to purchase should be grater than zero."))

        origin = ",".join([item.line_id.origin if item.line_id.origin else '' for item in self.item_ids])
        request_names = ",".join(self.item_ids.request_id.mapped('name'))
        if not purchase_id:
            purchase_id = self.env["purchase.order"].create(self._prepare_purchase_order(company_id,origin,picking_type_id))
            purchase_id.y_purchase_request_number = request_names
        ...
        for item in self.item_ids:
            ...
            # po line
            domain = self._get_order_line_search_domain(purchase_id, item)
            ...
            if available_po_lines and not item.keep_description:
                ...
                po_line_product_uom_qty = po_line.product_uom._compute_quantity(po_line.product_uom_qty, alloc_uom)
                if wizard_uom != alloc_uom:
                    wizard_product_uom_qty = wizard_uom._compute_quantity(item.quantity, alloc_uom)
                else:
                    wizard_product_uom_qty = item.quantity
                all_qty = wizard_product_uom_qty
                po_line.product_qty = all_qty + sum(po_line.mapped('product_uom_qty'))
            else:
                po_line_data = self._prepare_purchase_order_line(purchase_id, item)
            ...
            new_qty = pr_line_obj._calc_new_qty(line, po_line=po_line, new_pr_line=new_pr_line)
...
class PurchaseRequestLineMakePurchaseOrderItem(models.TransientModel):
    ...
    quantity = fields.Float(string="Quantity to purchase")
