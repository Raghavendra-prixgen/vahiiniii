...
class StockLandedCostNew(models.Model):
    ...
    def button_validate(self):
        ...
        for cost in self:
            ...
            # lot_move_to_add = defaultdict(dict) # Custom Code
                    ...
                    if line.move_id.product_id.lot_valuated:
                        for lot_id, sml in line.move_id.move_line_ids.grouped('lot_id').items():
                            ...
                            if product.cost_method in ['average', 'fifo']:
                                ...
                                # lot_move_to_add[sml].update({sml.lot_id:value}) # Custom Code
                            vals_list.append({
                                'value': value,
                                'unit_cost': 0,
                                'quantity': 0,
            ...
            for lot, value in cost_to_add_bylot.items():
                if lot.product_id.lot_valuated:
                    if float_is_zero(lot.quantity_svl, precision_rounding=lot.product_id.uom_id.rounding):
                        continue
                    lot.sudo().with_context(disable_auto_svl=True).standard_price += value / lot.quantity_svl


            # Custom Code
            # for sml,sm_lot in lot_move_to_add.items():
            #     sml_lot = list(sm_lot.keys())[0]
            #     value = list(sm_lot.values())[0]
            #     print(sml,sml_lot,value,")))))))))))))))))))")
            #     if sml.product_id.lot_valuated:
            #         if float_is_zero(sml_lot.quantity_svl, precision_rounding=sml.product_id.uom_id.rounding):
            #             continue
            #         sml_lot.sudo().with_context(disable_auto_svl=True).standard_price += value / sml_lot.quantity_svl
...
StockLandedCost.button_validate = StockLandedCostNew.button_validate