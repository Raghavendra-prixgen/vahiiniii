...
class CustomerPortalPartnerLedger(CustomerPortal):
    ...
    #     if sortby == 'date_asc':
    #         order = 'date asc'
    #         # Don't use 'amount' directly since it doesn't exist in the model
    #     # Use balance or debit/credit fields for amount-based sorting
    #     elif sortby == 'amount_desc':
    #         order = 'balance desc'  # or another valid field
    #     elif sortby == 'amount_asc':
    #         order = 'balance asc'  # or another valid field
    #     # elif sortby == 'amount_desc':
    #     #     order = 'debit desc, credit asc'
    #     # elif sortby == 'amount_asc':
    #     #     order = 'debit asc, credit desc'
    ...
    #         # Create a single dictionary with all the data, including the conditional amount_currency
    #         line_data = {
    #             'id': line.id,
    #             'date': line.date,
    #             'name': line.name,
    ...
    #         else:
    #             line_data['amount_currency'] = amount
    ...
    def _get_partner_ledger_values(self, page=1, date_begin=None, date_end=None, journal_type=None, outstanding_only=False, sortby=None, **kw):
        ...
        for line in ledger_lines:
            ...
            # Get matching number (from reconcile group or partial reconcile)
            matching_number = ""
            if line.full_reconcile_id:
                matching_number = line.full_reconcile_id.display_name
            elif line.matched_debit_ids or line.matched_credit_ids:
                # For partial reconciliation, use the first partial reconcile reference
                partial_refs = []
                if line.matched_debit_ids:
                    partial_refs.extend([p.display_name for p in line.matched_debit_ids])
                if line.matched_credit_ids:
                    partial_refs.extend([p.display_name for p in line.matched_credit_ids])
                if partial_refs:
                    matching_number = partial_refs[0]
            ...
            if not outstanding_only:
                line_data['amount_currency'] = line.amount_currency
                line_data['amount'] = round(amount,2)
            else:
    ...
    def portal_partner_ledger(self, page=1, date_begin=None, date_end=None, journal_type=None, outstanding_only=False, sortby=None, **kw):
        ...
        # values['initial_loading'] = True  # Add this flag for initial loading state
